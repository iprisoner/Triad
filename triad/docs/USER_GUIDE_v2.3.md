# Triad Station v2.3 用户指南
## 纯文本认知编排 · 本地 LLM 推理 · 使用手册
...
Triad Station 是一台运行在你本地 WSL2 环境中的**纯文本认知编排 AI 工作站**。它不是网页聊天框，而是一个拥有三层架构的操作系统级 Agent 集群：

- **🦞 龙虾控制台**：聊天、Agent 可视化、VRAM 监控、模型配置、技能市场
- **📊 系统监控**：GPU 显存、Docker 容器、llama-server 状态、CPU/内存

> **v2.3 变更**: 移除了 ComfyUI 多模态画布。Triad v2.3 聚焦纯文本+代码认知编排，图像/视频生成未来通过 MCP 插件扩展。

**核心特性**：
- **@角色指令**：输入 `@novelist` 或 `@code_engineer` 切换专业角色
- **动态模型**：不限于 6 个厂商，支持无限添加模型，Web UI 直接配置
- **显存跷跷板**：llama.cpp `-ngl` 参数实现 GPU/CPU 自动切换
...
> **v2.3**: ComfyUI Tab 已移除。多模态功能未来通过 MCP 插件接入。

**切换 Tab 时状态不丢失**：
- 龙虾的 WebSocket 在后台继续运行
- 切回龙虾时，最新消息立即显示
...
### 2.3 系统监控（原 Tab 3，现 Tab 2）

实时查看系统状态，每 3 秒自动刷新。

> **v2.3**: 原 ComfyUI 画布 Tab 已移除。系统聚焦纯文本+代码场景。视觉生成未来通过 MCP 插件接入。
...
| **美术导演** | `@art_director` | 概念设计、视觉描述文档生成 | Grok/Gemini | 写代码、操作 git |
...
**生成概念设计文档**：
```
@art_director 为上面那位黑客女主角设计视觉概念文档。
外貌：银发紫瞳，身材纤细，左臂有神经接口疤痕。
风格：赛博朋克，电影级光影。
输出：详细的视觉描述，包含服装、姿态、环境氛围，可直接交给画师或未来 ComfyUI MCP 插件。
```

> **v2.3**: `@art_director` 现在仅生成文本视觉描述，不再直接调用图像生成。
...
```bash
# 1. 下载 triad-v2.3.tar.gz 并解压
cd ~
tar xzf triad-v2.3.tar.gz
cd triad
...
> **v2.3 变更**: 以下 ComfyUI 相关 FAQ 已过时。ComfyUI 多模态层已于 v2.3 移除，未来通过 MCP 插件接入。
...
A: **不会。** v2.3 已移除 ComfyUI 多模态层。所有角色都只生成纯文本输出。未来可通过 MCP 插件扩展图像生成。

**Q: 动态评估是怎么判断的？**
A: 系统从 5 个维度依次判断：
1. 角色有没有显式标记 `eval_strategy`？
2. `task_type` 是什么？（novel / code）
3. 角色 ID 的语义（含 "novel" → 小说评估；含 "code" → 代码评估）
4. 用户输入关键词
5. 生成内容的启发式推断（前 2000 字含 3+ 小说标记 → 降级评估）

v2.3 移除了多模态路由。如果都没命中，就 `bypass`（静默跳过），不浪费时间和资源。

**Q: 刷新浏览器后任务会丢失吗？**
A: **不会。** v2.3 已修复此问题。Gateway 会记录每个任务的阶段历史和最终结果到内存（最近 100 个任务）。重新连接后发送 `{"action": "recover_tasks"}`，服务器会把所有任务历史推送回来，UI 自动重建进度界面。

**Q: 蜂群任务会不会把显存搞崩？**
A: **不会。** v2.3 已引入 VRAM 全局锁：`vram_scheduler` 维护活跃 LLM 推理计数器。如果有 Agent 正在调用本地模型，其他 Agent 请求 VRAM 切换时会阻塞等待，直到所有推理安全完成。不会出现"写到一半被 SIGTERM 切断"的情况。

**Q: 3 个 Agent 各写 4000 字，合并后会不会超过上下文上限？**
A: **不会溢出。** v2.3 已引入 Map-Reduce 压缩：聚合结果超过 `max_output_tokens`（默认 6000）时，系统会自动调用一个轻量摘要 Agent，把 12000 字压缩到 6000 字以内，然后再传给下游节点。
...
- [x] **架构精简：移除 ComfyUI + Claude Code，聚焦 OpenClaw+Hermes+llama.cpp**

### 下一步（v2.4 / v3.0）
- [ ] Web UI 直接添加自定义角色（不用改代码）
- [ ] 语音输入/输出集成
- [ ] 云端同步备份（可选）
- [ ] **MCP 插件生态：ComfyUI、浏览器、语音等通过标准 MCP 接口接入（v3.0）**
...
| ~~ComfyUI 注入~~ | ~~`hand/comfyui_mcp_bridge.py`~~ | **v2.3 已移除** |

---

*本文档面向 Triad Station v2.3 用户，基于系统真实状态编写。*