...
│  │                     监听 0.0.0.0:18188, Docker 经 host.docker.internal 访问         ││
│  │                                                                                  ││
│  │   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐                       ││
...
│   └── MCP Bridge（调用 ComfyUI API via host.docker.internal:18188）
├── WSL2 宿主机原生：
│   └── ComfyUI（Python venv，监听 0.0.0.0:18188）
│       ├── SDXL + LoRA
...
**通信路径**：
- Docker 容器 → `host.docker.internal:18188` → WSL2 宿主机 ComfyUI
...
3. **验证 MCP Bridge 连接**
   - Bridge 启动时 health_check 验证 `host.docker.internal:18188` 可达
