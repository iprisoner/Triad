...
┌──────────────────────────────────────────────────────────────────────────────┐
│                           多模态执行层                                          │
│  ├─ llama-server (Docker/本地) : -ngl 99/0 GPU/CPU 跷跷板 (:18000)             │
│  ├─ vram_scheduler_llama.py    : 显存状态机 (IDLE→CPU_FALLBACK→RENDERING)          │
│  └─ asset_manager.py           : asset:// URI + 版本链 + 缩略图                   │
│                                                                  │
│  > **v2.3 变更**: ComfyUI 多模态层已移除。图像/视频生成未来通过 MCP 插件接入。     │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
> **v2.3 变更**: 移除了 ComfyUI Tab。v2.3 聚焦纯文本认知编排，多模态通过外部 MCP 扩展。
...
> **v2.3**: ComfyUI Tab 已移除。系统聚焦纯文本+代码场景。
...
| `@art_director` | 概念设计，视觉描述文档生成 | CREATIVE | asset_search/write | 0.9 |
...
created_at: 2026-05-06T14:30:52
...
### 4.2 多模态任务流 [已在 v2.3 移除]

> **v2.3 变更**: ComfyUI 多模态执行层已移除。`@art_director` 角色现在仅生成文本描述和概念设计文档，不再触发图像生成。未来通过 MCP 插件重新接入 ComfyUI。

```
用户: "@art_director 设计赛博朋克女主角概念"
  → parse_role → art_director, model_pref=CREATIVE
  → router.execute() → 生成角色设定文本 + 视觉描述文档
  → _get_multimodal_strategy(...) → "bypass" (v2.3 默认不触发视觉)
  → report_result("success", 文本概念设计)
```
...
**动态多模态路由 (Step 5)** [已在 v2.3 简化]:

> v2.3 移除 ComfyUI 后，多模态路由默认始终返回 `"bypass"`。视觉生成未来通过 MCP 插件接入。

```
_get_multimodal_strategy(task_type, user_prompt, role, generated_text)
  → v2.3: 默认 → "bypass" (ComfyUI 已移除)
  → 未来 MCP 扩展: 通过外部插件触发
```

**关键收益**:
- 写代码时不弹出"小说质量评估"进度条
- `@novelist 写第一章` 不触发不必要的视觉流程
- v2.3 架构更精简，VRAM 调度仅用于 llama.cpp GPU/CPU 切换
- 系统总吞吐量提升（无 ComfyUI VRAM 跷跷板开销）
...
| ~~ComfyUI Bridge~~ | ~~`comfyui_mcp_bridge.py`~~ | ~~1597~~ | ~~已移除~~ | ~~v2.3 移除，未来 MCP 扩展~~ |
...
| **VRAM 全局锁已修复（地雷 1）** | `vram_scheduler_llama.py` 新增 `_llm_inference_counter` + `_vram_switch_lock` + `begin/end_llm_inference()` | ✅ v2.3 |
| **上下文压缩已修复（地雷 2）** | `swarm_orchestrator.py` 新增 `_estimate_tokens()` + `_compress_aggregated()` Map-Reduce 压缩 | ✅ v2.3 |
| **配方语义去重已修复（地雷 3）** | `skill_crystallizer.py` 新增 `_find_similar_recipe()` + `_merge_recipe()` 适者生存 | ✅ v2.3 |
| **WebSocket 状态恢复已修复（地雷 4）** | `websocket.ts` 新增 `taskHistoryStore` + `handleRecoverTasks()` + `recordTaskStage/Result()` | ✅ v2.3 |

---

*本文档基于对 /mnt/agents/output/triad/ 目录下 79 个文件、15,619 行代码的全面审计，生成于 2026-05-06。*026-05-03。*